...
void main() {
    ...
    // Reflection from mirror pass
    vec3 reflection = texture(uReflectionTex, fs_in.vTex).rgb;

    // Perturb reflection coordinates by normal
    vec2 reflCoord = fs_in.vTex + normal.xz * 0.05;
    reflection = mix(reflection, texture(uReflectionTex, reflCoord).rgb, 0.3);
    ...
    // Combine: interpolate between water color and reflection based on Fresnel
    vec3 finalColor = mix(waterColor, reflection, fresnel * 0.7);

    // Add specular highlight for wave peaks
    vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
    vec3 halfDir = normalize(lightDir + viewDir);
    float spec = pow(max(dot(normal, halfDir), 0.0), 64.0);
    finalColor += vec3(0.5) * spec * (1.0 - depthFactor);
